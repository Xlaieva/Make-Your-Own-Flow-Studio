...
async function main() {
  ...
  const defaultKb = await prisma.knowledgeBase.upsert({
    ...
    create: {
      name: '默认知识库',
      description: '系统自动创建的默认知识库，包含 Make Your Own Flow Studio 的功能介绍。',
  ...
  // 3. 为默认知识库创建一篇文档
  const docContent = `
Make Your Own Flow Studio 是一个先进的全栈可视化 AI 应用低代码编排平台。它旨在降低 AI 应用开发的门槛，使开发者和业务人员能够通过直观的拖拽式交互，快速构建、测试和部署复杂的 AI 工作流。
...
- AI: 通义千问 API (Qwen)
  `;
  const docName = 'Make Your Own Flow Studio 功能介绍.md';
  ...
  const demoNodes = [
    {
      ...
      data: {
        ...
        variables: [
          {
            key: 'question',
            value: 'Make Your Own Flow Studio 有什么核心特性？',
          },
        ],
      },
    },
